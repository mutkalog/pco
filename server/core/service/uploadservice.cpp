...
UploadService::UploadService(const fs::path &bufferDir, const fs::path storageDir)
    : bufferDir_(bufferDir), storageDir_(storageDir)
{}
                           ...
                           const std::string &archive)
{
    ...
    fs::path updateBufDir = bufferDir_ / ss.str();
    fs::path storagePath;
    ...
    try
    {
        ...
        parseFiles(sc, archive, entry, updateBufDir, storagePath);
...
void UploadService::parseFiles(std::shared_ptr<ServerContext> &sc, const std::string &raw, ReleasesTableEntry &entry, const fs::path& bufDir, fs::path& storagePath)
{
    ...
    if (fs::create_directories(storageDir_) == false && fs::exists(storageDir_) == false)
        throw std::runtime_error("Cannot create storage dir");

    if (sc->archiveTools->extract(data.data(), data.size(), bufDir.c_str()) != 0)
    {
    ...
    for (const auto& file : fs::directory_iterator(bufDir))
    {
        ...
        {
            fs::path serverPath = storageDir_ / entry.type / entry.arch / entry.platform / entry.version;
