...
{
    return std::make_unique<pqxx::connection>("dbname=" + dbname_ +
                                              " user=" + username_ +
                                              " host=" + host_ +
                                              " port=" + port_);
}
...
Database::Database(const std::string &dbname, const std::string &username,
    ...
    , port_(port)
{
    ...
    {
        {
            pqxx::connection sysBaseConn("dbname=postgres user=" + username +
                                         " host=" + host + " port=" + port);

            pqxx::work txn(sysBaseConn);
            auto res = txn.exec("SELECT 1 FROM pg_database WHERE datname=" +
                                txn.quote(dbname_));
            txn.commit();

            if (res.empty()) {
                pqxx::nontransaction ntx(sysBaseConn);
                ntx.exec("CREATE DATABASE " + dbname_);
                std::cout << "Database created: " << dbname_ << std::endl;
            }
