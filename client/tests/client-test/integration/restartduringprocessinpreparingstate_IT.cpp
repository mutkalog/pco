...
class RestartDuringProcessInPreparingStateIntegrationTest : public IntegrationFixtureBase
{
protected:
...
TEST_F(RestartDuringProcessInPreparingStateIntegrationTest, RestartWhilePreparingResumesAndCompletes)
{
    const int MAX_STEPS_TO_PREPARING = 6;
    int steps = 0;

    while (stateMachine->state() != StateExecutor::PREPARING &&
           ++steps < MAX_STEPS_TO_PREPARING)
    {
        stateMachine->run();
    }

    ASSERT_LT(steps, MAX_STEPS_TO_PREPARING);
