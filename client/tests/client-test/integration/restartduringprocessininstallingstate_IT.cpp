...
class RestartDuringProcessInInstallingStateIntegrationTest : public IntegrationFixtureBase
{
protected:
...
TEST_F(RestartDuringProcessInInstallingStateIntegrationTest, RestartWhileInstallingResumesAndCompletes)
{
    const int MAX_STEPS_TO_INSTALLING = 7;
    int steps = 0;

    while (stateMachine->state() != StateExecutor::INSTALLING &&
           ++steps < MAX_STEPS_TO_INSTALLING)
    {
        stateMachine->run();
    }

    ASSERT_LT(steps, MAX_STEPS_TO_INSTALLING);
