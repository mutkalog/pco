...
class RestartDuringProcessInVerifyingStateIntegrationTest : public IntegrationFixtureBase
{
protected:
...
TEST_F(RestartDuringProcessInVerifyingStateIntegrationTest, RestartWhileVerifyingResumesAndCompletes)
{
    const int MAX_STEPS_TO_VERIFY = 5;
    int steps = 0;
    while (stateMachine->state() != StateExecutor::VERIFYING && ++steps < MAX_STEPS_TO_VERIFY)
    {
        stateMachine->run();
    }

    ASSERT_LT(steps, MAX_STEPS_TO_VERIFY);
