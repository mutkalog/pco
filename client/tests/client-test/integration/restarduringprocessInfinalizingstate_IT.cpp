...
class RestartDuringProcessInFinalizingStateIntegrationTest : public IntegrationFixtureBase
{
protected:
...
TEST_F(RestartDuringProcessInFinalizingStateIntegrationTest, RestartWhileFinalizingResumesAndCompletes)
{
    const int MAX_STEPS_TO_FINALIZING = 9;
    int steps = 0;

    while (stateMachine->state() != StateExecutor::FINALIZING &&
           ++steps < MAX_STEPS_TO_FINALIZING)
    {
        stateMachine->run();
    }

    ASSERT_LT(steps, MAX_STEPS_TO_FINALIZING);
