...
void CheckingStateExecutor::execute(StateMachine &sm)
{
    ...
    std::string queryString = httplib::encode_uri(
                                    std::string("/manifest?id=") + std::to_string(ctx.devconf->id())
...
void CheckingStateExecutor::process(StateMachine &sm, const std::string& responseBody)
{
    ...
    if (ctx.cryptoUtils->verifySignature(
            ...
            ctx.devconf->publicKeyPath()) == false)
    {
    ...
    if (verificateRelease(ctx.manifest, ctx.devconf.get()) == true)
    {
        sm.transitTo(DOWNLOADING);
    }
    else
    {
...
bool CheckingStateExecutor::verificateRelease(const ArtifactManifest &received, const IClientConfig *current) const
{
