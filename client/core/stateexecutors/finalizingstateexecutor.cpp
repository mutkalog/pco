...
void FinalizingStateExecutor::execute(StateMachine &sm)
{
    ...
    safeExec([&] {
        if (ctx.rollback == true) {
            ...
            ctx.devconf->saveNewUpdateInfo(ctx.manifest);
...
void FinalizingStateExecutor::rollback(UpdateContext &ctx)
{
    ...
    {
        launchScript(ctx, ctx.stagingDir / it->installPath.filename());
    }
}

void FinalizingStateExecutor::launchScript(UpdateContext& ctx, const fs::path& scriptPath)
{
    if (ctx.syscalls->chmod(scriptPath.c_str(), 0755) != 0)
        throw std::runtime_error("Cannot chmod rollback.sh");

    pid_t childPid = 0;
    char* argv[2] = {const_cast<char*>(scriptPath.c_str()), nullptr};

    if (ctx.syscalls->posix_spawn(&childPid, scriptPath.c_str(), nullptr, nullptr, argv, environ) != 0)
    {
...
void FinalizingStateExecutor::totalCleanup(UpdateContext &ctx)
{
    ...
    {
        std::error_code ec;
        fs::remove_all(ctx.stagingDir, ec);
        if (ec)
